...
//! the input that lets the NAT classifier return `Easy`. Run this on a host
//! reachable from outside the machine under test's NAT (a public IP), not
//! on the same LAN: the probe only trusts an observation when the helper
//! reports the machine's public address, which an on-LAN helper never sees.
...
/// # Errors
///
/// Returns an error if either helper fails to bind its socket or generate a
/// certificate, or if the ctrl-c signal cannot be installed.
pub async fn nat_helper(bind: IpAddr, ports: (u16, u16)) -> Result<()> {
    ...
    println!();
    println!("where <this-host> is a public address this machine is reachable at");
    println!("from outside the tested NAT (an on-LAN helper cannot measure it).");
